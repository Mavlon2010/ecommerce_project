...
async def successful_payment_handler(message: Message):
    ...
    await message.answer(
        text=(
            ...
            f"💳 To'lov miqdori: {payment_info.total_amount / 100:,.0f} so'm\n"
            f"📦 Buyurtma raqami: {payment_info.invoice_payload}\n\n"
            f"Tez orada operatorlarimiz siz bilan bog'lanishadi."
        ),
        reply_markup=InlineKeyboardBuilder().button(
            text="🏠 Bosh menyu",
    ...
    if not results:
        results.append(
            InlineQueryResultArticle(
                ...
                description=f"“{query}” bo‘yicha mahsulotlar topilmadi.",
                input_message_content=InputTextMessageContent(
